  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          {arquivo ? (
            ...
            <div>
              <p className="font-medium text-slate-600">Arraste a planilha aqui ou clique para selecionar</p>
              <p className="text-sm text-slate-400 mt-1">Suporta .xls, .xlsx e .csv</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
        <p className="text-xs font-semibold text-slate-500 uppercase mb-3">Formato esperado da planilha</p>
      ...
      {resultado && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-5 space-y-3">
          ...
          <div className="flex gap-6 text-sm">
            <div>
              <span className="text-2xl font-bold text-green-700">{resultado.criados}</span>
              <p className="text-green-600">Alunos criados</p>
            </div>
            <div>
              <span className="text-2xl font-bold text-blue-700">{resultado.atualizados}</span>
              <p className="text-blue-600">Registros atualizados</p>
            </div>
          </div>

          {resultado.colunas_detectadas && (
            <div className="bg-white rounded-lg border border-green-200 p-3">
              <p className="text-xs font-semibold text-slate-500 mb-2">Colunas detectadas na planilha:</p>
              <div className="grid grid-cols-2 gap-1 text-xs">
                {Object.entries({
                  ...
                  <div key={label} className="flex gap-1">
                    ...
                    <span className={valor ? "text-green-700 font-medium" : "text-red-400"}>
            ...
            <div className="bg-red-50 rounded-lg border border-red-200 p-3">
              ...
              {resultado.erros.length > 20 && (
                <p className="text-xs text-red-400 mt-2 font-medium">
                  ...e mais {resultado.erros.length - 20} erros. Verifique as colunas detectadas acima.
                </p>
              )}
            </div>
          )}
      ...
      <div className="flex gap-3">
        ...
        {arquivo && !loading && (
          <button
            onClick={() => { setArquivo(null); setResultado(null); setErro(""); }}
